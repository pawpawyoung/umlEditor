...
void MainWindow::create_tool_bars()
{
    ...
    textLayout->addWidget(new QLabel("文本"), 1, 0, Qt::AlignCenter);
    QWidget *textWidget = new QWidget;
    textWidget->setLayout(textLayout);

    QGridLayout *arrowLayout = new QGridLayout();
    arrowLayout->addWidget(pointerButton, 0, 0, Qt::AlignHCenter);
    arrowLayout->addWidget(new QLabel("移动"), 1, 0, Qt::AlignCenter);
    ...
    arrowLayout2->addWidget(new QLabel("依赖关系"), 1, 0, Qt::AlignCenter);
    ...
    arrowLayout3->addWidget(new QLabel("联合会"), 1, 0, Qt::AlignCenter);
    ...
    arrowLayout4->addWidget(new QLabel("聚集作用"), 1, 0, Qt::AlignCenter);
    ...
    arrowLayout5->addWidget(new QLabel("组成"), 1, 0, Qt::AlignCenter);
    ...
    arrowLayout6->addWidget(new QLabel("概括"), 1, 0, Qt::AlignCenter);
    QWidget *arrowWidget6 = new QWidget;
    arrowWidget6->setLayout(arrowLayout6);
    layout->addWidget(arrowWidget6,4,1);

    layout->addWidget(createCellWidget("等级",My_diagram_item::Class_),0,0);
    layout->addWidget(textWidget, 0, 1);
    layout->addWidget(createCellWidget("接口",My_diagram_item::Interface),1,0);
...
//
    ...
    //
     ...
     layout2->addWidget(createCellWidget("演员",My_diagram_item::Actor),0,0);
     layout2->addWidget(createCellWidget("使用方案",My_diagram_item::Option),0,1);
     ...
     one_button_->setToolTip("简单的移动对象");
     ...
     tab2layout->addWidget(new QLabel("移动"), 1, 0, Qt::AlignCenter);
     ...
     tab2layout2->addWidget(new QLabel("概括"), 1, 0, Qt::AlignCenter);
     ...
     tab2layout3->addWidget(new QLabel("依赖关系"), 1, 0, Qt::AlignCenter);
     ...
     tab2layout4->addWidget(new QLabel("接通"), 1, 0, Qt::AlignCenter);
     ...
     tab2layout5->addWidget(new QLabel("扩张"), 1, 0, Qt::AlignCenter);
     ...
     tab2layout6->addWidget(new QLabel("联合"), 1, 0, Qt::AlignCenter);
     ...
     one_button_state->setToolTip("简单的移动对象");
     ...
     tab2layout_state->addWidget(new QLabel("移动"), 1, 0, Qt::AlignCenter);
     ...
     six_button_state->setToolTip("交易");
     ...
     tab2layout_state_2->addWidget(new QLabel("过渡"), 1, 0, Qt::AlignCenter);
     ...
     layout3->addWidget(createCellWidget("简单状态",My_diagram_item::State),0,0);
     layout3->addWidget(createCellWidget("初始状态",My_diagram_item::BeginState),0,1);
     layout3->addWidget(createCellWidget("终态",My_diagram_item::EndState),1,0);
     layout3->addWidget(createCellWidget("同步状态",My_diagram_item::SyncState),1,1);
     layout3->addWidget(createCellWidget("最近的",My_diagram_item::Shallow_history_state),2,0);
     layout3->addWidget(createCellWidget("很久以前",My_diagram_item::Deep_history_state),2,1);
     layout3->addWidget(createCellWidget("平行跃迁",My_diagram_item::Transition),3,0);
...
void MainWindow::on_Open_triggered()
{
   // QString fileName = QFileDialog::getOpenFileName(this);
    QString fileName = QFileDialog::getOpenFileName(this,"打开文件","","Diagram (*.dia)");
    if (!fileName.isEmpty())
        loadFile(fileName);
}
...
bool MainWindow::maybeSave()
{
    ...
    if (scene->items().size() != 0 && (scene->ismodified || (elms_on_scene != scene->items().size())))/*|| scene->items().size()*/
    {
        ...
        ret.setText("文件已更改。你想保存更改吗？");
        ret.addButton(QMessageBox::Save);
        ret.addButton(QMessageBox::Discard);
        ret.addButton(QMessageBox::Cancel);
        ret.setButtonText(QMessageBox::Save,"保存");
        ret.setButtonText(QMessageBox::Discard,"不保存");
        ret.setButtonText(QMessageBox::Cancel,"取消");
        int reft = ret.exec();
        switch (reft)
        {
...
bool MainWindow::saveAs()
{
    ...
    QString fileName = QFileDialog::getSaveFileName(this,"保存文件","","Diagram (*.dia)");
    if (fileName.isEmpty())
        return false;
...
//    return true;
    ...
    {
        QMessageBox::warning(this, tr("我的申请"),
                             tr("读取文件%1时出错: \n%2")
                             .arg(fileName)
...
//    QTextStream out(&file);
    ...
    ui->statusBar->showMessage("文件已保存", 2000);
...
//    setDirty(false);
    ...
    {
        QMessageBox::warning(this, tr("我的应用程序"),
                             tr("无法写入文件 %1:\n%2.")
                             .arg(fileName)
                             .arg(file.errorString()));
        return;
